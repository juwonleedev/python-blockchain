...
# 블록체인 네트워크에 필요한 함수들 - 블록 해시, 거래내역 저장, 채굴
...
def mine(): 
    ...
    blockchain.new_transaction(
        sender = mine_owner, #채굴 시 생성되는 transaction
        recipient = node_identifier, #지갑 주소처럼 사용
        amount = mine_profit #coinbase transaction 코인 1개
    )
    ...
    for node in blockchain.nodes:
        ...
        for node in blockchain.nodes:
            headers = {'Content-Type': 'application/json; charset=utf-8'}
            data = {
                "minder_node": 'https://' + my_ip + ":" + my_port,
            }

            a = requests.get("https://" + node + "/nodes/resolve", headers = headers, data = json.dupms(data))

            # print (a.text)
            # 이상이 없으면 정상 제출 
            if "ERROR" not in a.text :
                print ("다른 노드가 내 블록 검증, 결과 정상!!!!")
                # block이 제대로 mine 되었다는 정보를 json 형태로 띄워 줌
                response = {
